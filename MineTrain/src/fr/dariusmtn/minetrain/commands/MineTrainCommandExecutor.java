						...
						if (player.hasPermission("minetrain.admin.create")) {
							if (!plugin.editor.containsKey(player)) {
								...
								if (args.length == 1) {
									...
									if (linesnb == 0) {
										if (player.hasPermission("minetrain.admin.create.line")) {
								...
								// if arg is line or station
									...
									if (args.length == 2) {
										...
										// Create a new station
											...
											if (player.hasPermission("minetrain.admin.create.station")) {
					...
					if (subcmd.equalsIgnoreCase("list")) {
						if (player.hasPermission("minetrain.admin.list")) {
							...
							for (Line line : plugin.getFileManager().getLines()) {
								...
								// Edit
								if (player.hasPermission("minetrain.admin.edit") && player.hasPermission("minetrain.admin.edit.line"))
									fm.then(" ").then("[✏]").color(ChatColor.AQUA).tooltip("§cEdit this line")
											.command("/minetrainconfig editline " + line.getLineId().toString());
								// Delete
								if (player.hasPermission("minetrain.admin.remove"))
									fm.then(" ").then("[✕]").color(ChatColor.RED).tooltip("§cDelete this line")
											.command("/minetrainconfig deleteline " + line.getLineId().toString());
								// Send
								fm.send(player);
							}
							// Stations
							...
							for (Station station : plugin.getFileManager().getStations()) {
								...
								// Edit
								if (player.hasPermission("minetrain.admin.edit") && player.hasPermission("minetrain.admin.edit.station"))
									fm.then(" ").then("[✏]").color(ChatColor.AQUA).tooltip("§cEdit this station")
											.command("/minetrainconfig editstation " + station.getStationId().toString());
								// Delete
								if (player.hasPermission("minetrain.admin.remove"))
									fm.then(" ").then("[✕]").color(ChatColor.RED).tooltip("§cDelete this station")
											.command("/minetrainconfig deletestation " + station.getStationId().toString());
								// Send
								fm.send(player);
							}
							return true;
